...
class DashboardPageWidget(QWidget):
    ...
    def _build_cards(self) -> None:
        ...
        hrow.addWidget(self._btn_schedule, 0, Qt.AlignVCenter)
        hb.addLayout(hrow)
        ...
        self._tile_overdue.clicked.connect(self.open_schedule_requested.emit)
    ...
    def _install_bento_grid(self) -> None:
        """One-shot grid layout (no resize relayout — avoids transient top-level flashes)."""
        self._grid.addWidget(self._header_card, 0, 0, 1, 2)
        self._grid.addWidget(self._kpi_host, 1, 0, 1, 2)
        self._grid.addWidget(self._health_card, 2, 0)
        self._grid.addWidget(self._dept_card, 2, 1)
        self._grid.addWidget(self._next_card, 3, 0)
        self._grid.addWidget(self._attention_card, 3, 1)
        self._grid.addWidget(self._notes_card, 4, 0, 1, 2)
    ...
    def _update_next_7_days(self, snap: DashboardSnapshot) -> None:
        ...
        for item in rows[:10]:
            ...
            rl.setContentsMargins(6, 5, 6, 5)
            rl.setSpacing(8)
            dept_color = _RED if item.overdue else _COLOR_PROGRESS
            rl.addWidget(_chip(item.department_label, dept_color, row), 0, Qt.AlignVCenter)
            kind = "Shot" if item.entity_kind == "shot" else "Asset"
            name = QLabel(f"{kind} · {item.entity_name}", row)
            name.setFont(monos_font("Inter", 12, QFont.Weight.DemiBold))
            name.setStyleSheet("color: #e4e4e7; background: transparent;")
            rl.addWidget(name, 1)
