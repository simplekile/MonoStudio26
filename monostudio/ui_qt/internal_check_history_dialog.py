"""Internal check history dialog (internal_check_meta.json)."""
...
from monostudio.core.internal_check_reader import load_internal_check_history
...
class InternalCheckHistoryDialog(MonosDialog):
    _COL_NAME = 0
    _COL_ADDED_AT = 1
    _COL_DESCRIPTION = 2
    _COL_PATH = 3
    ...
    def __init__(self, project_root: Path | None, parent=None) -> None:
        ...
        self.setWindowTitle("Internal check — History")
        self.setModal(False)
        root = QVBoxLayout(self)
        root.setContentsMargins(16, 16, 16, 16)
        root.setSpacing(12)
        title = QLabel("Staged for internal check", self)
        ...
        self._table.setHorizontalHeaderLabels(["Name", "Added at", "Description", "Path"])
    ...
    def set_context(self, project_root: Path | None) -> None:
        self._project_root = Path(project_root) if project_root else None
        self._load()
    ...
    def _load(self) -> None:
        ...
        if not self._project_root or not self._project_root.is_dir():
            return
        for e in load_internal_check_history(self._project_root):
