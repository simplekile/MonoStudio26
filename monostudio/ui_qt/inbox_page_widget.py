...
class InboxPageWidget(QWidget):
    """Inbox: sidebar Client/Freelancer + one explorer tree for that source."""
    ...
    def open_item_path(self, project_root: Path, item_path: Path) -> bool:
        """Select source/date folder and reveal a file or folder in the explorer."""
        loc = resolve_inbox_location(project_root, item_path)
        if loc is None:
            return False
        source, date_folder = loc
