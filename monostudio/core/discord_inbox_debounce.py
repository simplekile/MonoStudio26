"""Debounce inbox_received Discord webhooks — batch, reconcile on disk, persist outbox."""
...
from monostudio.core.discord_outbox import (
    list_inbox_received_entries,
    remove_inbox_received_entry,
    upsert_inbox_received_entry,
)
...
_log = logging.getLogger("monostudio.discord_inbox_debounce")

# Wait after the last drop before posting (avoids spam when importing many files).
INBOX_RECEIVED_DEBOUNCE_S = 45.0
...
def _parse_iso_dt(raw: str) -> datetime | None:
    ...
    try:
        ...
        return None
    # Naive timestamps in inbox_meta are local wall-clock; aware/Z values are absolute.
...
def resolve_project_root(workspace_root: Path | str, project_name: str) -> Path | None:
    """Resolve project folder from workspace + name (folder or display name)."""
    ...
    try:
        ...
        _log.debug("discover_projects failed during inbox reconcile", exc_info=True)
    return None


def reconcile_inbox_received(
    ...
    *,
    ...
    """
    Read inbox on disk — return count + names still present for this batch window.
    Uses inbox_meta added_at when available; falls back to pending_names that still exist.
    """
    ...
    inbox_root = get_inbox_root(root)
    date_dir = inbox_root / src / date
    if not date_dir.is_dir():
        return 0, []

    window_start = _parse_iso_dt(window_started_at)
    meta = read_inbox_meta(root)
    prefix = f"{src}/{date}/"
    names: list[str] = []
    seen: set[str] = set()
...
def _resolve_last_added_at(
    ...
    *,
    ...
    """Latest added_at among reconciled inbox files (UTC ISO Z)."""
    root = Path(project_root).resolve()
    src = (source or "client").strip().lower()
    date = (date_str or "").strip()
...
_buckets: dict[str, _InboxBatch] = {}
_timers: dict[str, threading.Timer] = {}


def _entry_to_batch(entry: dict) -> _InboxBatch | None:
...
def _dispatch_reconciled_batch(batch: _InboxBatch, *, bucket_key: str) -> bool:
    """Reconcile on disk, POST if non-empty. Returns True if dispatched."""
    count, names = reconcile_inbox_received(
        batch.project_root,
        source=batch.source,
    ...
    if count <= 0:
        _log.debug(
            "Discord inbox_received skipped after reconcile (empty) source=%s date=%s",
            batch.source,
            batch.date_str,
        )
        try:
            remove_inbox_received_entry(batch.workspace_root, bucket_key)
        except Exception:
            pass
        return False
    ...
    dispatch_discord_event(
        ...
        dedupe_key=f"inbox_received:{batch_id}",
    ...
    try:
        ...
        pass
    _log.debug(
        "Discord inbox_received flushed count=%s source=%s date=%s (reconciled)",
        count,
        batch.source,
        batch.date_str,
    )
...
def _outbox_entry_for_key(workspace_root: Path, bucket_key: str) -> dict[str, Any] | None:
    ...
    if not key:
        return None
    for entry in list_inbox_received_entries(workspace_root):
...
def _flush_from_outbox_entry(entry: dict[str, Any]) -> bool:
    ...
    if batch is None or not key:
        ...
        if ws and key:
            try:
                remove_inbox_received_entry(ws, key)
            except Exception:
                pass
        return False
    with _lock:
...
def _persist_batch(
    ...
    *,
    ...
    try:
        ...
        _log.debug("discord outbox persist failed", exc_info=True)


def enqueue_inbox_received(
    ...
    *,
    ...
    """Queue inbox_received; merged per project+source+date, sent after quiet period."""
    ...
    if proj is None:
        _log.debug("inbox_received enqueue skipped — project not resolved for %s", project_name)
        return

    delay = INBOX_RECEIVED_DEBOUNCE_S if debounce_s is None else max(1.0, float(debounce_s))
...
def flush_all_pending_inbox_received(workspace_root: Path | str | None) -> int:
    """Flush in-memory timers and persisted outbox immediately (app quit). Returns dispatch count."""
    ...
    for entry in list_inbox_received_entries(ws):
        key = str(entry.get("bucket_key") or "").strip()
...
def restore_inbox_received_outbox(workspace_root: Path | str | None) -> None:
    """
    On workspace open: reschedule debounce timers or flush overdue outbox entries.
    """
    if workspace_root is None:
        return
    ws = Path(workspace_root).resolve()
    ...
    for entry in list_inbox_received_entries(ws):
        ...
        with _lock:
            ...
            timer.start()
        _log.debug(
            "Restored inbox_received debounce key=%s remaining=%.1fs",
            key,
            remaining,
        )
